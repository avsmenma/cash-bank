...
@push('scripts')
    <script>
        (function () {
            ...
            var bkUndo = createUndoManager({
                table: table,
                getId: function (d) { return d.id_bank_keluar; },
                trackFields: ['agenda_tahun', 'no_bukti', 'tanggal_raw', 'id_sumber_dana', 'id_bank_tujuan', 'id_kategori_kriteria', 'id_sub_kriteria', 'id_item_sub_kriteria', 'id_jenis_pembayaran', 'penerima', 'uraian', 'kredit_raw', 'keterangan'],
                companions: ['sub_kriteria', 'item_sub_kriteria'],
                applyState: function (row, state, fields) {
                    var id = row.getData().id_bank_keluar;
                    var grid = {}; fields.forEach(function (f) { grid[f] = state[f]; });
                    if ('sub_kriteria' in state) grid.sub_kriteria = state.sub_kriteria;
                    if ('item_sub_kriteria' in state) grid.item_sub_kriteria = state.item_sub_kriteria;
                    row.update(grid);
                    var backend = { _token: csrf(), _method: 'PUT' };
                    fields.forEach(function (f) {
                        var v = state[f];
                        if (f === 'kredit_raw') backend.kredit = (v === '' || v == null ? 0 : v);
                    ...
                    $.ajax({ url: '/bank-keluar/' + id, type: 'POST', global: false, data: backend })
            ...
            table.on('cellEditing', function (cell) { bkUndo.beginEdit(cell.getRow()); });
            ...
            table.on('cellEdited', function (cell) {
                ...
                bkUndo.commitEdit(editedRow);   // rekam setelah cascade diterapkan
            ...
            document.addEventListener('keydown', function (e) {
                ...
                if ((e.ctrlKey || e.metaKey) && !e.shiftKey && (e.key === 'z' || e.key === 'Z')) { e.preventDefault(); bkUndo.undo(); return; }
                if ((e.ctrlKey || e.metaKey) && ((e.key === 'y' || e.key === 'Y') || (e.shiftKey && (e.key === 'z' || e.key === 'Z')))) { e.preventDefault(); bkUndo.redo(); return; }
            ...
            function bkPaste(text) {
                ...
                var _before = bkUndo.snapshotRows(order);
                order.forEach(function (id) { bkSaveRow(id, byRow[id]); });
                bkUndo.recordBatch(_before, bkUndo.snapshotRows(order));
            }
            ...
            function bkClearSelection() {
                ...
                var _before = bkUndo.snapshotRows(_ids);
                bkApplyTargets(targets);
                bkUndo.recordBatch(_before, bkUndo.snapshotRows(_ids));
                return true;
            }
