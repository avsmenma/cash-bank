<table>
    {{-- Kop laporan --}}
    <tr>
        <td colspan="11" style="font-weight: bold; font-size: 16px; color: #1E3A5F;">LAPORAN BANK MASUK</td>
    </tr>
    <tr>
        <td colspan="11" style="font-size: 11px; color: #6B7280;">{{ $filterInfo }} • {{ number_format($data->count(), 0, ',', '.') }} transaksi • diexport {{ now()->format('d/m/Y H:i') }}</td>
    </tr>
    <tr><td></td></tr>

    {{-- Header tabel (baris 4) --}}
    <tr>
        @foreach (['No', 'Agenda', 'Tanggal', 'Sumber Dana', 'Bank Tujuan', 'Kategori', 'Dari / Penerima', 'Uraian', 'Jenis Pembayaran', 'Debet (Rp)', 'Keterangan'] as $judul)
    ...
    @foreach ($data as $index => $item)
        ...
        <tr>
            ...
            <td style="{{ $bg }} border: 1px solid #B3BFCC; text-align: right;">{{ $item->debet + 0 }}</td>
    ...
    @if ($data->count())
        <tr>
            <td colspan="9" style="background-color: #1E3A5F; color: #FFFFFF; font-weight: bold; text-align: right; border: 1px solid #FFFFFF;">TOTAL</td>
            <td style="background-color: #1E3A5F; color: #FFFFFF; font-weight: bold; text-align: right; border: 1px solid #FFFFFF;">{{ $data->sum('debet') + 0 }}</td>
