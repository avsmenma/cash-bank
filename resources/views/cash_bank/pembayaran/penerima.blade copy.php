...
$('#createtPenerima').on('shown.bs.modal', function (event) {
     // Fungsi untuk menghitung
    function hitungNilai() {
        console.log('HITUNG NILAI DIPANGGIL');
        
        let volume = parseFloat($('#volume').val()) || 0;
        let harga  = parseFloat($('#harga').val()) || 0;

        console.log('Volume:', volume, 'Harga:', harga);

        let nilai = volume * harga;
        $('#nilai').val(nilai.toFixed(2));

        let ppn = Math.round(nilai * 0.11);
        $('#ppn').val(ppn);

        let potppn = parseFloat($('#potppn').val()) || 0;
        let nilaiInc = nilai + ppn - potppn;

        $('#nilai_inc_ppn').val(nilaiInc.toFixed(2));
        
        console.log('Hasil - Nilai:', nilai, 'PPN:', ppn, 'Nilai Inc:', nilaiInc);
    }

    // Event listener KHUSUS untuk input volume, harga, dan potppn
    $('#volume, #harga').on('input change', function () {
        console.log('INPUT BERUBAH:', $(this).attr('id'));
        hitungNilai();
    });

    $('#potppn').on('input change', function () {
        console.log('POTPPN BERUBAH');
        hitungNilai();
    });
    // Trigger saat modal dibuka
    $('#createPenerima').on('shown.bs.modal', function () {
        console.log('MODAL DIBUKA - RESET FORM');
        
        // Reset form
        $('#importExcel')[0].reset();
        
        // Inisialisasi ulang datepicker jika perlu
        if (!$('#reservationdate').data('datetimepicker')) {
    ...
    $('#createPenerima').on('hidden.bs.modal', function () {
        $('#importExcel')[0].reset();
    });
});
